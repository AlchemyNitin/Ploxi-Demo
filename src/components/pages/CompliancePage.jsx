...
const CompliancePage = () => {
  ...
  // Certificate filter groups
  const certFilters = [
    { id: 'all', label: 'All', group: null },
    // Green Building
    { id: 'BEE', label: 'BEE Star Rating', group: 'Green Building' },
    { id: 'GRIHA', label: 'GRIHA', group: 'Green Building' },
    { id: 'LEED', label: 'LEED', group: 'Green Building' },
    { id: 'WELL', label: 'WELL Accredited', group: 'Green Building' },
    // ESG Reporting
    { id: 'GRI', label: 'GRI', group: 'ESG Reporting' },
    { id: 'BRSR', label: 'BRSR', group: 'ESG Reporting' },
    { id: 'SASB', label: 'SASB', group: 'ESG Reporting' },
    { id: 'TCFD', label: 'TCFD', group: 'ESG Reporting' },
    // Environmental
    { id: 'ISO14001', label: 'ISO 14001', group: 'Environmental' },
    { id: 'CPCB', label: 'CPCB', group: 'Environmental' },
    { id: 'EIA', label: 'EIA', group: 'Environmental' },
  ];

  // Map cert IDs to task keywords for filtering
  const certKeywords = {
    BEE: ['bee', 'energy star', 'energy efficiency'],
    GRIHA: ['griha'],
    LEED: ['leed'],
    WELL: ['well', 'indoor air', 'health'],
    GRI: ['gri', 'global reporting'],
    BRSR: ['brsr', 'business responsibility'],
    SASB: ['sasb'],
    TCFD: ['tcfd', 'climate disclosure', 'climate-related'],
    ISO14001: ['iso 14001', 'iso14001', 'environmental management'],
    CPCB: ['cpcb', 'pollution control'],
    EIA: ['eia', 'environmental impact'],
  };
  ...
  // Filter tasks based on search, status, category, and certificate
  const filteredTasks = getAllTasks().filter(task => {
    ...
    let matchesCert = filterCert === 'all';
    if (!matchesCert && certKeywords[filterCert]) {
      const keywords = certKeywords[filterCert];
      const searchText = `${task.title} ${task.description} ${task.category || ''} ${(task.tags || []).join(' ')}`.toLowerCase();
      matchesCert = keywords.some(kw => searchText.includes(kw));
    }

    return matchesSearch && matchesStatus && matchesCategory && matchesCert;
  });
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200">
          ...
          <div className="p-6 border-b border-gray-200 space-y-4">
            ...
            {/* Row 2: Certificate Filters */}
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Filter className="w-4 h-4 text-gray-500" />
                <span className="text-sm font-medium text-gray-700">Filter by Certificate / Framework:</span>
                {filterCert !== 'all' && (
                  <button onClick={() => setFilterCert('all')} className="text-xs text-red-500 hover:underline ml-auto flex items-center gap-1">
              ...
              <div className="flex flex-wrap gap-2">
                {certFilters.map(cert => (
                  <button
                    key={cert.id}
                    onClick={() => setFilterCert(cert.id)}
                    className={`px-3 py-1 rounded-full text-xs font-medium transition-all border ${filterCert === cert.id
                      ? 'bg-green-600 text-white border-green-600'
                      : cert.group === 'Green Building'
                        ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100'
                        : cert.group === 'ESG Reporting'
                          ? 'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100'
                          : cert.group === 'Environmental'
              ...
              <div className="flex gap-4 mt-1 text-xs text-gray-400">
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-400 inline-block"></span>Green Building</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-blue-400 inline-block"></span>ESG Reporting</span>
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block"></span>Environmental</span>
              </div>
            </div>
          </div>
